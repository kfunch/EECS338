...
def bagOfWordsByParagraph(url):
	...
	for x in range(0, len(clean_text)):
		...
		parsed_text = nltk.word_tokenize(clean_text[x])
		#print parsed_text
		...
		if (len(parsed_text) > 50):
			...
			problemScores.append(-1)
			personalScores.append(-1)
			solutionScores.append(-1)
	...
	return [visible_text[problemScores.index(max(problemScores))],
			visible_text[personalScores.index(max(personalScores))],
			visible_text[solutionScores.index(max(solutionScores))]]
